...
async function main() {
  ...
  const characters = await Promise.all([
    prisma.character.create({
      data: {
        ...
        habits: JSON.stringify(["夜半抄录旧籍", "焦虑时以指节敲桌", "不喝热茶"]),
        catchphrase: "先别急着信命。",
        ...
        avatar: "裴",
        tags: JSON.stringify(["宿命", "禁术", "成长型主角"]),
      },
    }),
    prisma.character.create({
      data: {
        ...
        habits: JSON.stringify(["记录梦境", "说谎时会停顿半拍"]),
        ...
        avatar: "闻",
        tags: JSON.stringify(["冷感女主", "规则改写", "高智"]),
      },
    }),
    prisma.character.create({
      data: {
        ...
        habits: JSON.stringify(["拂袖掩笑", "说话前必轻叩骨扇三下"]),
        catchphrase: "棋局才刚摆好。",
        ...
        avatar: "白",
        tags: JSON.stringify(["反派", "谋局", "旧朝遗脉"]),
      },
    }),
  ]);
  console.log("Created characters:", characters.length);
  ...
  await Promise.all([
    prisma.chapterContent.createMany({
      data: [
        {
          id: "content-89-1",
          chapterId: "chapter-89",
          title: "塔门铜纹",
          content: "塔门上的铜纹像活了一样，从她的指尖一圈圈亮起。闻灯站在灰塔最外层的风廊上，听见身后司天卫整齐划一的脚步声。\n\n「退后三步。」她低声说。\n\n没有人动。\n\n铜纹已经爬到她的手肘，像是从皮肤下渗出来的光。她知道自己没有太多时间——灰塔的封印正在松动，旧王朝留在塔底的禁术刻痕已经苏醒。",
          orderIndex: 0,
        },
        {
          id: "content-89-2",
    ...
    prisma.aiSuggestion.createMany({
      data: [
        {
          id: "ai-sug-1",
          chapterId: "chapter-89",
          type: "continuation",
          label: "灰塔内部视角",
          content: "切换至塔内视角，描写禁术刻痕的激活过程。铜纹从塔基向塔尖蔓延，每一层封印都在发出低沉的共鸣。可以在这里插入一段关于旧王朝封缄仪式的回忆闪回。",
          tension: "high",
        },
        {
          id: "ai-sug-2",
          ...
          type: "conflict",
          label: "司天卫内部意见分歧",
          content: "在闻灯开门的过程中，司天卫内部出现分歧：一部分人认为应该保护闻灯完成仪式，另一部分人则认为灰塔开启过于危险。这个冲突可以为后续势力站队埋下伏笔。",
          tension: "medium",
        },
        {
          id: "ai-sug-3",
          ...
          type: "rewrite",
          label: "闻灯情感线改写",
          content: "当前版本中闻灯显得过于冷静。建议在她触摸铜纹的瞬间插入一段关于司天府流放的童年记忆，让她的情绪有一个短暂的失控瞬间，随后恢复理性。这样能增强角色立体感。",
          tension: "low",
        },
      ],
    }),
  ]);
  console.log("Created chapter contents, branches, suggestions");
      ...
      data: [
        {
          ...
          weather: "暴雨",
          timeSetting: "子夜",
          dangerZones: JSON.stringify(["祭井", "塌陷回廊"]),
          residents: JSON.stringify(["裴照夜", "闻灯"]),
        },
        {
          id: "scene-2",
          workId: work.id,
          ...
          weather: "阴云压城",
          timeSetting: "黎明前",
          dangerZones: JSON.stringify(["观测台边缘", "禁术刻痕区"]),
          residents: JSON.stringify(["司天卫", "白砚侯"]),
        },
      ],
    }),
    prisma.loreEntry.createMany({
      data: [
        {
          id: "lore-1",
          workId: work.id,
          ...
          linkedTitles: JSON.stringify(["海祠祭法", "亡潮季"]),
          riskLevel: "stable",
        },
        {
          id: "lore-2",
          ...
          linkedTitles: JSON.stringify(["灰塔封缄令", "司天一脉流放"]),
          riskLevel: "warning",
        },
      ],
    }),
  ...
  console.log("Created scenes, lore entries, factions");

  await prisma.factionEdge.create({
    data: {
      id: "fe-1",
      sourceId: "faction-1",
      targetId: "faction-2",
      edgeType: "对抗",
      note: "司天府与沉羽会在灰塔控制权上存在根本分歧",
    },
  });
  console.log("Created faction edges");

  await Promise.all([
    prisma.relationNode.createMany({
      data: [
    ...
    prisma.relationEdge.createMany({
      data: [
        {
          id: "edge-1-2",
          sourceId: "char-1",
  ...
  console.log("Created relation nodes and edges");
